...
const updateEnrollment = async (data, validate) => {
  validate.$touch();
  if (!validate.$invalid) {
    try {
      isLoading.value = true;
      await enrollmentStore.updateEnrollment(route.params.id, data);
...
const getEnrollment = async () => {
  try {
    ...
    await enrollmentStore.getEnrollment(route.params.id)
    createData.course_id = enrollmentStore.enrollment?.course_id;
    createData.user_id = enrollmentStore.enrollment?.user_id;
    createData.status = enrollmentStore.enrollment?.status;
  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">Update Enrollment</h1>
        <p class="mt-2 text-sm text-gray-700"> Update enrollment in admin side</p>
      </div>
