...
const adminNavigation = [
  ...
  { name: 'Students', href: 'students', icon: UsersIcon, routeName: 'students' },
  { name: 'Courses', href: 'courses', icon: FolderIcon, routeName: 'courses' },
  { name: 'Enrollments', href: 'enrollments', icon: InboxIcon, routeName: 'enrollments' },
]

const studentNavigation = [
  { name: 'Dashboard', href: 'dashboard', icon: HomeIcon, routeName: 'student-portal-dashboard' },
  { name: 'Courses', href: 'courses', icon: FolderIcon, routeName: 'student-portal-courses' },
  { name: 'Enrollments', href: 'enrollments', icon: InboxIcon, routeName: 'student-portal-enrollments' },
]
        ...
        <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
          <div class="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-gray-800">
            ...
            <div class="mt-5 flex-1 h-0 overflow-y-auto">
              <nav v-if="isAdmin" class="px-2 space-y-1">
              ...
              <nav v-else class="px-2 space-y-1">
                <RouterLink v-for="item in studentNavigation" :to="{ name: item.routeName }" :key="item.name" :class="[item.routeName === route.name ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'group flex items-center px-2 py-2 text-base font-medium rounded-md']">
                  <component :is="item.icon" :class="[item.routeName === route.name ? 'text-gray-300' : 'text-gray-400 group-hover:text-gray-300', 'mr-4 flex-shrink-0 h-6 w-6']" aria-hidden="true" />
                  {{ item.name }}
                </RouterLink>
              </nav>
    ...
    <div class="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0">
      ...
      <div class="flex-1 flex flex-col min-h-0 bg-gray-800">
        ...
        <div class="flex-1 flex flex-col overflow-y-auto">
          <nav v-if="isAdmin" class="flex-1 px-2 py-4 space-y-1">
          ...
          <nav v-else class="flex-1 px-2 py-4 space-y-1">
            <RouterLink v-for="item in studentNavigation" :to="{ name: item.routeName }" :key="item.name" :class="[item.routeName === route.name ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'group flex items-center px-2 py-2 text-sm font-medium rounded-md']">
              <component :is="item.icon" :class="[item.routeName === route.name ? 'text-gray-300' : 'text-gray-400 group-hover:text-gray-300', 'mr-3 flex-shrink-0 h-6 w-6']" aria-hidden="true" />
              {{ item.name }}
            </RouterLink>
          </nav>
